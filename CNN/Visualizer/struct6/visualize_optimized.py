...
import theano
from CNN.Visualizer.struct6.deconv_constructor import *
...
def Attention(classifier, x, nkerns, jpgfile):
    ...
    print Classify_pass(test_set_x)
    ...
    #     img.append(upsampled_img)
        ...
        #         fname = "imgs/deconv/struct6/L2_%i.png" % i
        #         # plt.imsave(fname, upsampled_img[0].transpose(1, 2, 0))
        #         plt.imsave(fname, upsampled_img[0][i], cmap='gray')
        #         plt.close()
    ...
    #             upsampled_img = f(Layer_inp)
    #
    #             fname = "imgs/deconv/struct6/full_%i.png" %c
    #             plt.imsave(fname,upsampled_img[0].transpose(1,2,0))
    #             plt.close()
...
def ForwardPass(classifier,x):
    output_layer = classifier.get_intermediate_sides()
...
def Classify(classifier,x):
    output_layer9 = classifier.layer9.y_pred  #layer9.inp
    # output_layer9 = classifier.layer9.p_y_given_x
    Test_model = theano.function(
        inputs=[x],
...
def construct_main_weights(classifier,nkerns):
    ...
    main_weights = []
    for idx in range(no_params-2,-1,-2):
